...
module.exports = class extends Base {
  ...
  getCart() {
    ...
    return _asyncToGenerator(function* () {
      const cartList = yield _this.model('cart').where({ user_id: _this.getLoginUserId(), session_id: 1 }).select();
      // 获取购物车统计信息
      let goodsCount = 0;
  ...
  checkoutAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (addressId) {
        checkedAddress = yield _this8.model('address').where({ is_default: 1, user_id: _this8.getLoginUserId() }).find();
      } else {
        checkedAddress = yield _this8.model('address').where({ id: addressId, user_id: _this8.getLoginUserId() }).find();
      }
