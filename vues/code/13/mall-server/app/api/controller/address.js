...
module.exports = class extends Base {
  ...
  listAction() {
    ...
    return _asyncToGenerator(function* () {
      const addressList = yield _this.model('address').where({ user_id: _this.getLoginUserId() }).select();
      let itemKey = 0;
  ...
  detailAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      const addressInfo = yield _this2.model('address').where({ user_id: _this2.getLoginUserId(), id: addressId }).find();
  ...
  saveAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (think.isEmpty(addressId)) {
        ...
        yield _this3.model('address').where({ id: addressId, user_id: _this3.getLoginUserId() }).update(addressData);
      }

      // 如果设置为默认，则取消其它的默认
      if (_this3.post('is_default') === true) {
        yield _this3.model('address').where({ id: ['<>', addressId], user_id: _this3.getLoginUserId() }).update({
          is_default: 0
        });
      }
  ...
  deleteAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      yield _this4.model('address').where({ id: addressId, user_id: _this4.getLoginUserId() }).delete();
